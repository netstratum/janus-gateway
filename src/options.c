...
gboolean janus_options_parse(janus_options *options, int argc, char *argv[]) {
	...
	GOptionEntry opt_entries[] = {
		...
		{ "node-number", 0, 0, G_OPTION_ARG_INT, &options->node_number, "Node number when multiple server node involved (default=1)", "number" },
		{ "server-instance", 0, 0, G_OPTION_ARG_INT, &options->server_instance, "server instace number when multiple janus instaces involved (default=1)", "number" },
		{ "app-put-url", 0, 0, G_OPTION_ARG_STRING, &options->app_put_url, "", "url" },
		{ "app-del-url", 0, 0, G_OPTION_ARG_STRING, &options->app_del_url, "", "url" },
		{ "app-cookie", 0, 0, G_OPTION_ARG_STRING, &options->app_cookie, "", "cookie" },
		{ "app-req-timeout", 0, 0, G_OPTION_ARG_INT, &options->app_req_timeout, "application registration request timeout in seconds", "number" },
		{ "ws-port", 0, 0, G_OPTION_ARG_INT, &options->ws_port, "websocket port", "port" },
		// { "http-port", 0, 0, G_OPTION_ARG_INT, &options->http_port, "http port", "port" },
		// { "admin-http-port", 0, 0, G_OPTION_ARG_INT, &options->admin_http_port, "admin http port", "port" },
		{ "ws-ip", 0, 0, G_OPTION_ARG_STRING, &options->ws_ip, "janus websocket ip", "ipaddress" },
